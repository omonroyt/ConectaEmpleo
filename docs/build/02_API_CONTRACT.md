# 02 — Contrato API (frontend mock ⇄ backend real)

Derivado de `docs/04` §5, §7, §8, §10 y `docs/05` §8. **El mock del frontend y el backend FastAPI deben cumplir exactamente este contrato.** Cambios: aditivos, anotados en la bitácora de `00_BUILD_STATE.md`.

## 1. Convenciones

- Base URL `/api/v1`. JSON. Auth `Authorization: Bearer <jwt>`. `role` viaja en el claim y en `User`.
- Errores: `{ code: string, message: string, details?: unknown }` con HTTP 4xx/5xx. El frontend muestra `message` en lenguaje accionable.
- Paginación: `?limit=&offset=` → `{ items: T[], total: number }`.
- Fechas ISO 8601 en UTC. IDs `uuid` string. Dinero en MXN entero mensual.
- Operaciones lentas devuelven `202 { job_id }` y se consultan con `GET /jobs/{id}`; el frontend hace polling cada 1.5 s y muestra `progress` con mensajes contextuales.
- Anonimización es estructural: `AnonymousCandidateCard` no tiene campos de identidad. `UnlockedCandidateProfile` solo existe tras `POST /match-results/{id}/unlock`.

## 2. Catálogo fijo (semilla compartida por mock y backend)

Familias y competencias (`code` · nombre · tipo · core):

**ADMIN_ASSISTANT — Auxiliar administrativo**
`OFFICE_TOOLS` Herramientas de oficina (Excel, Word, correo) · T · core
`DOCUMENT_CONTROL` Control documental y archivo · T · core
`SCHEDULING_COORDINATION` Agenda y coordinación · T
`CUSTOMER_SERVICE` Atención a clientes y proveedores · T · core
`WRITTEN_COMMUNICATION` Comunicación escrita · T
`ORGANIZATION_PRIORITIZATION` Organización y priorización · B · core
`TEAM_COLLABORATION` Colaboración en equipo · B
`PROBLEM_SOLVING` Resolución de problemas · B

**HEAVY_MACHINERY_OPERATOR — Obrero operador de maquinaria pesada**
`MACHINERY_OPERATION` Operación de maquinaria · T · core
`SAFETY_PROTOCOLS` Protocolos de seguridad · T · core
`PREVENTIVE_MAINTENANCE` Mantenimiento preventivo · T
`LOAD_HANDLING` Manejo de cargas · T · core
`SITE_SIGNALING` Señalización y comunicación en obra · T
`RISK_AWARENESS` Conciencia de riesgo · B · core
`INSTRUCTION_FOLLOWING` Seguimiento de instrucciones · B
`TEAM_COORDINATION` Coordinación con el equipo · B

**WAREHOUSE_SUPERVISOR — Encargado de almacén**
`INVENTORY_CONTROL` Control de inventarios · T · core
`FORKLIFT_SAFETY` Seguridad en montacargas · T · core
`WMS_ERP_SYSTEMS` Sistemas WMS / ERP · T
`RECEIVING_DISPATCH` Recepción y despacho · T · core
`STORAGE_ORGANIZATION` Organización de almacén · T
`TEAM_COORDINATION` Coordinación de equipo · B · core
`PROBLEM_SOLVING` Resolución de problemas · B
`DISCREPANCY_RESOLUTION` Resolución de diferencias · B

Pesos por defecto del matching (`MatchComponent`): TECHNICAL 40 · BEHAVIORAL 20 · EXPERIENCE 15 · EVIDENCE 10 · SALARY 8 · LOCATION 7. Siempre normalizados a 100.

## 3. Tipos TypeScript (`src/api/types.ts`, copiar literal)
...
## 4. Endpoints ⇄ `ApiClient` (`src/api/client.ts`)

| Método `ApiClient` | HTTP | Notas |
|---|---|---|
| `auth.register(RegisterInput)` → `AuthResponse` | `POST /auth/register` | |
| `auth.login(LoginInput)` → `AuthResponse` | `POST /auth/login` | |
| `auth.me()` → `User` | `GET /auth/me` | |
| `catalog.jobFamilies()` → `JobFamily[]` | `GET /job-families` | |
| `catalog.competencies(jobFamilyId)` → `Competency[]` | `GET /job-families/{id}/competencies` | |
| `catalog.skills()` → `Skill[]` | `GET /skills` | |
| `candidate.me()` / `candidate.update(patch)` → `CandidateProfile` | `GET/PATCH /candidates/me` | |
| `candidate.setJobFamily(jobFamilyId)` → `CandidateProfile` | `POST /candidates/me/job-family` | |
| `candidate.status()` → `CandidateStatusView` | `GET /candidates/me/status` | decide "siguiente paso" en home |
| `candidate.talentProfile()` → `TalentProfile` | `GET /candidates/me/talent-profile` | 404 si no existe |
| `candidate.skills()` → `CandidateSkill[]` | `GET /candidates/me/skills` | |
| `candidate.feedback()` → `FeedbackReport` | `GET /candidates/me/feedback` | A4 |
| `candidate.learningPath()` → `LearningPath` | `GET /candidates/me/learning-path` | A4 |
| `documents.uploadCV(file)` → `JobRef` | `POST /candidates/me/cv` (multipart) | 202 |
| `documents.extraction()` → `CVExtraction` | `GET /candidates/me/cv/extraction` | |
| `documents.confirmExtraction(patch)` → `CVExtraction` | `PATCH /candidates/me/cv/extraction` | marca confirmada → status CV_READY |
| `documents.uploadCertification(file, skillCode?)` → `DocumentRef` | `POST /candidates/me/certifications` | |
| `cvBuilder.createSession()` → `CVBuilderReply` | `POST /cv-builder/sessions` | primer mensaje del agente |
| `cvBuilder.sendMessage(id, text)` → `CVBuilderReply` | `POST /cv-builder/sessions/{id}/messages` | |
| `cvBuilder.finalize(id)` → `CVExtraction` | `POST /cv-builder/sessions/{id}/finalize` | crea extracción para revisión |
| `cvBuilder.document(id)` → `CVDocument` | `GET /cv-builder/sessions/{id}/document` | CV descargable |
| `interviews.create(mode)` → `InterviewSession` | `POST /interviews` | reutiliza sesión IN_PROGRESS si existe |
| `interviews.get(id)` → `InterviewSession` | `GET /interviews/{id}` | |
| `interviews.nextQuestion(id)` → `NextQuestion` | `GET /interviews/{id}/next-question` | síncrono, ≤ 45 s |
| `interviews.answer(id, AnswerInput)` → `NextQuestion` | `POST /interviews/{id}/answers` | devuelve la siguiente pregunta |
| `interviews.progress(id)` → `InterviewProgress` | `GET /interviews/{id}/progress` | |
| `interviews.complete(id)` → `JobRef` | `POST /interviews/{id}/complete` | job INTERVIEW_EVALUATE + PROFILE_BUILD |
| `interviews.turns(id)` → `InterviewTurn[]` | `GET /interviews/{id}/turns` | historial (Should Have) |
| `company.me()` / `company.update(patch)` → `Company` | `GET/PATCH /companies/me` | |
| `company.verification()` → `VerificationView` | `GET /companies/me/verification` | visual en MVP |
| `vacancies.create(VacancyInput)` → `Vacancy` | `POST /vacancies` | |
| `vacancies.list()` → `Vacancy[]` | `GET /vacancies` | de la empresa actual |
| `vacancies.get(id)` / `vacancies.update(id, Partial<VacancyInput> & {status?})` → `Vacancy` | `GET/PATCH /vacancies/{id}` | |
| `vacancies.setRequirements(id, RequirementInput[])` → `Vacancy` | `PUT /vacancies/{id}/requirements` | backend normaliza pesos |
| `vacancies.setWeights(id, VacancyWeights)` → `Vacancy` | `PUT /vacancies/{id}/weights` | normaliza a 100 |
| `vacancies.resolveRequirements(id, freeText)` → `RequirementResolution` | `POST /vacancies/{id}/resolve-requirements` | A5 RESOLVE; devuelve advertencias discriminatorias |
| `vacancies.runMatch(id)` → `JobRef` | `POST /vacancies/{id}/match-runs` | 202 |
| `matching.results(runId, {limit, offset})` → `Paginated<AnonymousCandidateCard>` | `GET /match-runs/{id}/results` | ordenado por score desc |
| `matching.result(matchResultId)` → `AnonymousCandidateCard` | `GET /match-results/{id}` | incluye `explanation_text` |
| `matching.unlock(matchResultId)` → `UnlockedCandidateProfile` | `POST /match-results/{id}/unlock` | escribe `candidate_unlocks` |
| `matching.fullProfile(matchResultId)` → `UnlockedCandidateProfile` | `GET /match-results/{id}/full` | 403 sin unlock |
| `matching.compare(vacancyId, matchResultIds[])` → `CompareView` | `GET /vacancies/{id}/compare?ids=` | máximo 3 |
| `matching.shortlist(vacancyId)` → `ShortlistEntry[]` | `GET /vacancies/{id}/shortlist` | |
| `matching.setShortlistStage(matchResultId, stage \| null)` → `ShortlistEntry` | `PUT /match-results/{id}/shortlist` | null = quitar |
| `opportunities.list()` → `Opportunity[]` | `GET /vacancies/open` | candidato; compatibilidad calculada con su perfil |
| `opportunities.get(vacancyId)` → `Opportunity` | `GET /vacancies/open/{id}` | |
| `opportunities.apply(vacancyId)` → `Application` | `POST /vacancies/{id}/apply` | |
| `opportunities.myApplications()` → `Application[]` | `GET /candidates/me/applications` | |
| `jobs.get(id)` → `Job` | `GET /jobs/{id}` | polling |
| `mocks.notifications()` / `mocks.messages()` / `mocks.plans()` | `GET /notifications` · `GET /messages` · `GET /billing/plans` | P2 |

Voz (fase backend): `WS /interviews/{id}/voice` (audio del candidato → STT → respuesta → TTS streaming). En el frontend se abstrae con `VoiceGateway` (§6).

## 5. Reglas del mock (`src/api/mock`)

- Latencia simulada 300–900 ms; jobs avanzan `progress` en 3–4 ticks (~4 s total) y terminan `DONE`. `localStorage` persiste sesión, perfil, vacantes y desbloqueos bajo la clave `ce-mock-v1` para que la demo sobreviva recargas. Función `resetMock()` expuesta en `window.__ce.resetMock` en dev.
- Semilla (`api/mock/seed/`): 3 familias con competencias de §2 · catálogo de ~40 skills · **5 candidatos evaluados por familia** (anon codes `CND-xxxx`, nombres mexicanos realistas solo visibles tras unlock, evaluaciones con justificaciones que citan respuestas, mezcla declarada/evaluada/verificada, distintas bandas geográficas y salariales) · 1 empresa demo verificada "Logística del Bajío S.A. de C.V." · 1 vacante OPEN por familia con requisitos y pesos · match runs precalculados por vacante · notificaciones, hilos y 3 planes.
- Usuarios demo: `candidato@demo.mx` / `demo1234` (perfil nuevo, status DRAFT, para recorrer el golden path) · `maria@demo.mx` / `demo1234` (candidata ya EVALUATED con talent profile, familia WAREHOUSE_SUPERVISOR) · `empresa@demo.mx` / `demo1234`.
- Entrevista mock: banco de 6–8 preguntas por familia, con al menos una `PROBE` cuyo texto cite la respuesta anterior (`references_turn_id` no nulo) — ej. "Mencionaste que conciliabas diferencias cada semana; cuéntame de una vez que el físico no cuadró con el sistema, ¿qué hiciste?". `question_budget` = 6 en mock para que la demo dure ~4 min. `complete` genera un `TalentProfile` con scores plausibles derivados de la longitud/keywords de las respuestas.
- CV builder mock: guion de 8 turnos del A1 (último trabajo, actividades, herramientas, trabajos previos, estudios, certificaciones, disponibilidad/zona/salario) y `finalize` arma la extracción.
- `resolveRequirements` mock: mapea por keywords al catálogo de la familia, detecta y devuelve como `warnings` frases como "máximo 30 años", "solo hombres", "buena presentación".
- Matching mock: implementa la fórmula real `Σ(peso_i × score_i) − penalizaciones` con los pesos de la vacante, para que la explicación coincida con el desglose. `explanation_text` por plantilla a partir del breakdown (máx. 120 palabras, nunca un porcentaje distinto al `total_score`).

## 6. `VoiceGateway` (`src/voice`)

```ts
export interface VoiceGateway {
  readonly available: boolean;                           // false → UI ofrece solo texto
  ...
  stopListening(): Promise<{ transcript: string }>;      // transcript "" si no hubo STT
  dispose(): Promise<void>;
}
```

`BrowserVoiceGateway` (fase frontend, modo mock): TTS con `window.speechSynthesis` en `es-MX` (sin analyser → Orb `speaking` procedural); STT con `webkitSpeechRecognition` si existe, si no devuelve transcript vacío y la UI muestra el campo de texto; micrófono vía `getUserMedia` + `connectAudio(stream)` para alimentar el Orb en `listening`. `ServerVoiceGateway` (fase backend) implementará la misma interfaz sobre el WebSocket. La entrevista siempre muestra la pregunta en texto y permite responder por texto; la voz es opcional (`mode`).
