...
import { Badge, Button, Card, ProgressBar, Reveal, RevealGroup, SkeletonCard, useToast } from "@/components/ui";
...
export function VacancyDetailPage() {
  ...
  if (vacancyQuery.isError || !vacancyQuery.data) {
    return (
      <PageContainer className="py-10">
        <p className="text-sm text-danger-on-dark">No pudimos cargar esta vacante. Intenta recargar la página.</p>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer className="py-10">
      <div className="flex flex-col gap-8">
        ...
        <RevealGroup className="grid gap-6 lg:grid-cols-3" as="div">
            ...
            <Reveal>
              <Card variant="light" padding="lg">
                ...
                <div className="mt-4 flex flex-col gap-4">
                  {WEIGHT_COMPONENT_ORDER.map((component, index) => (
                    <ProgressBar
                      key={component}
          ...
          <div className="flex flex-col gap-6">
            <Reveal>
              <Card variant="glass" padding="lg">
                <h2 className="text-base font-semibold text-text-on-dark">Matching</h2>
                {vacancy.last_match_run_id ? (
                  <>
                    <p className="mt-2 text-sm text-text-on-dark-secondary">Ya generamos un ranking de talento para esta vacante.</p>
                    <Button
                      variant="primary"
                      arrow
                  ...
                  <>
                    <p className="mt-2 text-sm text-text-on-dark-secondary">Aún no has ejecutado el matching para esta vacante.</p>
            ...
            <Reveal>
              <Card variant="glass" padding="lg">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="text-base font-semibold text-text-on-dark">Finalistas</h2>
                  <Link
                    to={`/employer/vacancies/${vacancy.id}/shortlist`}
                    className="text-sm font-medium text-primary-on-dark hover:underline"
                  >
                    Ver selección
                  </Link>
                </div>
                {shortlist.length === 0 ? (
                  <p className="mt-3 flex items-center gap-2 text-sm text-text-on-dark-secondary">
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                    {(Object.keys(STAGE_LABELS) as ShortlistStage[]).map((stage) => (
                      <div key={stage} className="rounded-md bg-white/[0.06] p-3">
                        <p className="text-xl font-semibold text-text-on-dark">{stageCounts[stage]}</p>
                        <p className="text-xs text-text-on-dark-secondary">{STAGE_LABELS[stage]}</p>
                      </div>
                    ))}
                  </div>
                )}
